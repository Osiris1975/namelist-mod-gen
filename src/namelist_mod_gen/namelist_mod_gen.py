...
NAME_LIST_OUTPUT = os.path.join(ROOT_DIR, 'output_namelists')

template_loader = FileSystemLoader(searchpath=TEMPLATES_DIR)
template_env = Environment(loader=template_loader)
namelist_template = template_env.get_template(NAMELIST_TEMPLATE)
...
def render(args):
    ...
    file_dest = os.path.join(NAME_LIST_OUTPUT, f"{namelist_dict2['namelist_id']}.txt")
...
def get_template_variables():
    variables = jinja2schema.infer(Path(os.path.join(TEMPLATES_DIR, "namelist.txt")).read_text())
